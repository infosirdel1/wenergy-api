...
export default async function handler(req, res) {
  ...
  // --- Firestore direct fallback: update by request_number from PaymentIntent.description ---
  const pi = event.data?.object;
  const requestNumber = pi?.description; // ex: "S00025"

  if (requestNumber) {
    try {
      const snapByReq = await firestore
        .collection("requests")
      ...
      if (!snapByReq.empty) {
        const docByReq = snapByReq.docs[0];
        const dataByReq = docByReq.data();

        if (dataByReq.payment_status !== "paid") {
          await docByReq.ref.update({
            payment_status: "paid",
            paid_at: new Date(),
        ...
        return res.status(200).json({ received: true, updated: "by_request_number" });
      }
    } catch (err) {
      console.error("Firestore update by request_number failed", err?.message || err);
      // continue to Odoo fallback
    }
  }
  // --- end Firestore direct fallback ---
  ...
  try {
    const orderResp = await axios.post(
      `${ODOO_URL}/web/dataset/call_kw`,
      {
        ...
        params: {
          ...
          method: "read",
          args: [[orderId]],
          kwargs: { fields: ["state", "name", "invoice_ids"] },
        },
        id: Date.now(),
      },
      { headers: { Cookie: cookieHeader } }
    ...
    if (Array.isArray(orderList) && orderList.length > 0) {
      orderState = orderList[0].state;
      orderName = orderList[0].name;
      invoiceIds = orderList[0].invoice_ids;
      console.log("SALE ORDER:", { orderState, orderName, invoiceIds });
    }
  } catch (err) {
